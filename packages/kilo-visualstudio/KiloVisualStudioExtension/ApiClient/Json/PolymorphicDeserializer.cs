  ...
  public static class PolymorphicDeserializer
  {
    ...
    public static object DeserializeMessage(JToken token, JsonSerializer serializer)
    {
      ...
      return role switch
      {
        ...
        "assistant" => DeserializeAssistantMessage(token, serializer),
        _ => throw new JsonSerializationException($"Unknown Message role '{role}'")
      };
    }

    public static AssistantMessage DeserializeAssistantMessage(JToken token, JsonSerializer serializer)
    {
      var result = (token as JObject).ToObject<AssistantMessage>(serializer);
      var error = token["error"];
      if (error != null)
        result.Error = DeserializeError(token, serializer);
      return result;
    }
