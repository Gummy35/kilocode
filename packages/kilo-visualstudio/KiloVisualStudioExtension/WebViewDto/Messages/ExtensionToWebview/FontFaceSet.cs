// <auto-generated>
//     This code was generated by WebViewContractGenerator.
//     Do not modify this file directly as changes will be lost on regeneration.
//     Source: WebViewContract.json schema version 1.0.0
// </auto-generated>

#nullable enable

namespace KiloVisualStudioExtension.WebView.Generated;

using System;
using System.Collections.Generic;
using System.Threading.Tasks;
using Newtonsoft.Json;

/// <summary>
/// WebView message: FontFaceSet
/// Discriminator: status = "loading"
/// Source: ..\..\..\..\..\kilo-visualstudio\tools\webview-contract-extractor\node_modules\typescript\lib\lib.dom.d.ts
/// </summary>
public class FontFaceSet
{
    [JsonProperty("onloading", NullValueHandling = NullValueHandling.Ignore)]
    // Original TypeScript type: null | __type
    public object? Onloading { get; set; }
    [JsonProperty("onloadingdone", NullValueHandling = NullValueHandling.Ignore)]
    // Original TypeScript type: null | __type
    public object? Onloadingdone { get; set; }
    [JsonProperty("onloadingerror", NullValueHandling = NullValueHandling.Ignore)]
    // Original TypeScript type: null | __type
    public object? Onloadingerror { get; set; }
    [JsonProperty("ready")]
    // Original TypeScript type: Promise
    public Task<object> Ready { get; set; }
    [JsonProperty("status")]
    public string Status { get; set; }
}
