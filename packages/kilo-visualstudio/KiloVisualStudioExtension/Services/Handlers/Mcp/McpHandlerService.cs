...
namespace KiloVisualStudioExtension.Services.Handlers.Mcp
{
  ...
  public class McpHandlerService : ServiceProviderServiceBase
  {
    ...
    /// 2. If no client, send cached MCP status
    ...
    public async Task HandleRequestMcpStatusAsync()
    {
      try
      {
        ...
        if (nswagClient == null)
        {
          var cached = cacheService.Get<McpStatusLoadedMessage>("mcpStatusLoadedMessage");
          if (cached != null)
            Provider.PostMessage(cached);
          return;
        }

        var directory = _serviceProvider.GetService<ProjectDirectoryProvider>().GetWorkspaceDirectory();
        var data = await Retry.RetryAsync(() => nswagClient.Mcp_statusAsync(directory, ""));
        if (data != null)
        {
          var message = new McpStatusLoadedMessage
          {
            Status = data
          };
          await cacheService.UpdateAsync("mcpStatusLoadedMessage", message);
