...
namespace KiloVisualStudioExtension
{
  ...
  public class VSProvider : ServiceProviderServiceBase, IDisposable
  {
    ...
    ///// </summary>
    //private async Task CheckpointAsync(string sid, Func<Task> run)
    //{
    ...
    ///// </summary>
    //private async Task WaitForCheckpointAsync(string sid)
    //{
    ...
    internal async Task SendImageModelsAsync(List<KiloModelsImagesResponseSchema200Item> models)
    {
    ...
    internal async Task DisposeGlobalAsync()
    {
...
*/
    ...
    private async Task ProcessMessageAsync(string type, IWebviewMessage message)
    {
      try
      {
        ...
        if (await EarlyMessageRouter.RouteWebviewMessageAsync(message,
        ...
        if (message is IEditorActionMessage editorActionMessage
          && await _serviceProvider.GetService<UiService>().HandleEditorOpenMessageAsync(editorActionMessage)) return;
        ...
        if (await SidebarWorktreeHandler.HandleWorktreeMessageAsync(message, new SidebarWorktreeHandler.Ctx
        {
          Post = (msg) => PostMessage(msg),
      ...
      //  if (await this.handleModelSelectorExpandedMessage(message)) return
        ...
        switch (type)
        {
          ...
          case "requestIndexingSettings":
            await _settingsService.SendIndexingSettingsAsync();
            break;

          case "requestChatSettings":
            await _settingsService.SendChatSettingsAsync();
            break;
