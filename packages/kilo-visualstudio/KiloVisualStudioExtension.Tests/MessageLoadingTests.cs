...
namespace KiloVisualStudioExtension.Tests
{
    public class MessageLoadingTests
    {
        ...
        public void HandleLoadMessages_DeletedSession_DropsResponse()
        {
            ...
            sseHelper.TrackSession("session-to-delete");
            sseHelper.IsSessionTracked("session-to-delete").Should().BeTrue();

            // Delete the session (untrack it)
            sseHelper.UntrackSession("session-to-delete");

            // Act - try to load messages for deleted session
            var isTracked = sseHelper.IsSessionTracked("session-to-delete");
